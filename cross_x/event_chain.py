...
E-CS makes the configured Cross-X chain observable as ordered handoffs. It does
not run subsystems: every event is a plan record carrying the subsystem
contract, predecessor, return boundary, and governance state.
"""
...
def freeze_ecs_mapping(value: Mapping[str, Any]) -> Mapping[str, Any]:
    """Return a recursively immutable mapping for E-CS return history."""

    if not isinstance(value, Mapping):
        raise TypeError("E-CS value must be a mapping")
    return MappingProxyType({str(key): _freeze_ecs_value(item) for key, item in value.items()})
...
def normalize_ecs_identifier(value: object, *, field: str) -> str:
    ...
    E-CS identifiers can be embedded in W3Lgu/Cross-X packets, so packet
    delimiters such as comma, colon, newline, and whitespace are rejected.
    """
...
class EventChainRecord:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "event_id", normalize_ecs_identifier(self.event_id, field="event_id"))
        object.__setattr__(self, "chain_id", normalize_ecs_identifier(self.chain_id, field="record.chain_id"))
        if self.return_value is not None:
            object.__setattr__(self, "return_value", freeze_ecs_mapping(self.return_value))
        object.__setattr__(
            self,
            "return_history",
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "status": self.status,
            "return_value": thaw_ecs_mapping(self.return_value) if self.return_value is not None else None,
            "return_history": [thaw_ecs_mapping(item) for item in self.return_history],
...
class EventChain:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "chain_id", normalize_ecs_identifier(self.chain_id, field="chain_id"))
        object.__setattr__(self, "supervisor", normalize_ecs_identifier(self.supervisor, field="supervisor"))
...
def build_event_chain(
    *,
    ...
    events = []
    for index, system in enumerate(normalized_tuple, start=1):
        ...
        events.append(
            EventChainRecord(
                ...
                predecessor=normalized_tuple[index - 2] if index > 1 else None,
                successor=normalized_tuple[index] if index < len(normalized_tuple) else None,
...
def bind_event_return(
    ...
    *,
    ...
    """Return a new E-CS chain with one event result bound.

    The original chain remains unchanged. This is the safe return path for
    adapters or observers; it records results without letting E-CS execute.
    """
    ...
    if normalized_status not in {"completed", "waiting", "stopped", "inactive"}:
        raise ValueError("E-CS return status must be completed, waiting, stopped, or inactive")
    ...
    if current.status not in {"planned", "waiting"}:
        raise ValueError(f"E-CS event status {current.status!r} cannot accept a return value")

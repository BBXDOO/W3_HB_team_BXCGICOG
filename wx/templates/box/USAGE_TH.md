...
ใน W3 คำว่า “คู่มือ” ไม่ได้หมายถึงเอกสารที่ต้องรอให้ระบบสมบูรณ์ก่อนจึงเขียนได้

คู่มือคือการบันทึกความคิดแบบมีโครงสร้างในจังหวะที่ความคิดกำลังเกิด กำลังขยาย หรือกำลังมี “กระบวนการสร้างอยู่ในหัว”

```text
คู่มือ = ภาพรวมทิศทาง + เหตุผล + วิธีมอง + วิธีเริ่มใช้
ไม่ใช่เอกสารที่ต้องมีของครบทุกชิ้นก่อน
```

เหตุผล:

```text
วันที่ “ครบจริง” มักไม่มีอยู่จริง
เพราะเมื่อครบแล้ว จะมีสิ่งที่จริงกว่า ลึกกว่า หรือขยายกว่าเกิดขึ้นเสมอ
```

ดังนั้นคู่มือของ `wx:BOX` ควรช่วยให้ผู้อ่านเห็นภาพทันทีว่า:

```text
- สิ่งนี้เกิดมาเพื่ออะไร
- ทำไมต้องมี
- ใช้อย่างไรแบบไม่หนัก
- ขอบเขตอยู่ตรงไหน
- ควรเริ่มจากจุดใด
```

คู่มือไม่ควรเขียนให้กลัว ไม่ควรทำให้คนต้องรอบท 1, 2, 3 แบบแข็งตัว และไม่ควรทำให้แนวคิดสดใหม่ตายเพราะรอความสมบูรณ์
...
```yaml
refs:
  identity: core/identity/profiles/ChatGPT.idp.json
